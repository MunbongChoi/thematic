...
def build_samgeo(args: argparse.Namespace, device: torch.device) -> object:
    try:
        from samgeo import SamGeo
    except ImportError as exc:
        raise ImportError(
            "SAM inference now uses segment-geospatial. Install it with: "
            "pip install segment-geospatial"
        ) from exc
    kwargs = {"model_type": args.samgeo_model_type, "automatic": False}
    if args.samgeo_checkpoint:
...
def read_samgeo_mask(mask_path: Path, image_size: tuple[int, int]) -> np.ndarray:
    try:
        ...
        with Image.open(mask_path) as mask_image:
            return np.asarray(mask_image.convert("L").resize(image_size, Image.NEAREST), dtype=np.uint8) > 0
...
def run_sam(args: argparse.Namespace, device: torch.device) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="samgeo_") as temp_name:
        ...
        for image_path in iter_images(Path(args.input)):
            ...
            for segment in prompt_segments:
                ...
                refined.update({"id": next_segment_id, "area": area, "bbox": mask_bbox(mask), "prompt_source": "mask2former_samgeo"})
